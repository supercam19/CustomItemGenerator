...
window.generatePack = function() {
    ...
    let mcmeta = pack.file("pack.mcmeta", JSON.stringify(packmeta))
    let namespace = data.folder(item_name)
    ...
    let func_load = functions.file("load.mcfunction", "scoreboard objectives add ${item_name}_rcd minecraft.used:minecraft.${base_item}")
    let func_tick = functions.file("tick.mcfunction", "execute as @a if score @s ${item_name}_rcd matches 1.. run ${right_click_method}")
    ...
    if (on_hit_action == "commands") {
        let func_on_hit = functions.file("on_hit.mcfunction", document.getElementById("on_hit-commands").value)
    ...
    if (override_item != "none") {
        let true_base_item = override_item
    }
    else {
